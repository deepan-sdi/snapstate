	...
	public function changePasswordAction()
	{
		...
		if ($request->isPost()) {
			...
			if ($changePasswordForm->isValid()) {
				if(md5($formData['password']) == $userSession->userSession['password']) {
					$document	= array('$set' => array('password' => md5(addslashes($formData['newpassword']))));
					$query		= array('_id' => $userSession->userSession['_id']);
					$conn		= $this->connect();
					$collection	= $conn->snapstate->users;
					$collection->update($query, $document);
					$userSession->userSession['password']	= md5($formData['newpassword']);
